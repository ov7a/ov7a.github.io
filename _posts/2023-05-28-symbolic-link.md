...
Признак того, что файл является символической ссылкой, хранится в поле `mode` в inode: у обычных файлов это `010xxxx` (в 8-ричной системе; `xxxx` — это unix-права вместе со sticky битом), а у символических ссылок — `012xxxx`. А путь ссылки хранится в данных. Т.е. ссылка — это просто текстовый файл, у которого выставлен специальный флаг.
