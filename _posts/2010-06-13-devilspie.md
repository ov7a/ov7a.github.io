...
*Эту статью я изначально писал на хабр, но там она не вышла из песочницы и инвайт я не получил. Но я решил написать одному из администраторов русскоязычного сообщества Ubuntu, [Malamut](https://forum.ubuntu.ru/index.php?action=profile;u=20962). И в итоге получилась wiki-страница на help.ubuntu.ru, а я получил от Malamut инвайт на хабр.*
...
(matches a b)
```
Выполняется, когда строка а соответствует регулярному выражению b.
Пример:
```
...
### Прикрепить список контактов Gaim Buddylist ко всем рабочим столам с размером 340×630 пикселей и с положением x=4, y=150. 
```
(if
    (and 
...
```
Окно диалоговое, и просто так развернуть его во весь экран не получится. Поэтому сначала ему назначаем тип. В отличие от первого случая, Devil's Pie будет постоянно отслеживать окно и не будет завершаться после применения правила.
