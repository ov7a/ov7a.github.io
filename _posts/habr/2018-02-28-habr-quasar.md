...
```
Свойство <code>co.paralleluniverse.fibers.verifyInstrumentation</code> отвечает за проверку в рантайме корректности манипуляций с байт-кодом. Разумеется, если эта проверка включена, то все начинает тормозить:)
