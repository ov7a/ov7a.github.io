...
7. [Hamcrest](http://hamcrest.org/JavaHamcrest/) ­— фаворит многих автотестировщиков по мнению [Яндекса](https://habr.com/ru/company/yandex/blog/346186/). Поверх нее еще работает [valid4j](http://www.valid4j.org/).
...
13. [HamKrest](https://github.com/npryce/hamkrest) — как Hamcrest, только HamKrest (на самом деле от Hamcrest только название и стиль).
...
* Интересная запись для ловли исключения:
    ...
    ```
* Описания ошибок в целом отличные, но нет подробностей отличия двух коллекций, что не так. Еще расстроила ошибка в формате `Expected Iterable to contain none of "[1, 3]"` — непонятно, что на самом деле проверяемый Iterable содержит.
* Интеграция: `<Click to see difference>` есть только для простых ассертов.
...
* Описание ошибки: нет подробностей для словаря и массивов, а в целом довольно подробно.
* Сложные структуры: тест с вложенными массивами не прошел.
</details>
