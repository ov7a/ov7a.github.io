...
tags: [koka, search, фп, frontend, blogdev, c, unicode]
category: blog
tg_id: 639
---
...
Мы же в чистом ФП, при каждом запросе запрашивать и парсить JSON — отвратительно, т.е. его надо передавать как параметр к функции.
...
Первая попытка запуска поиска была, разумеется, неуспешная, потому что
```
...
В первой итерации я сделал обычный TF-IDF.
...
```
т.е. от мутабельного зависеть там нельзя.
Но в итоге и эта реализация тоже оказалась медленной.
Даже в консоли это было медленное говно, медленнее любой из предыдущих реализаций.
...
~~На стадии экспериментов выяснил, что индексер работает медленнее при `-O3`, но это некритично, 20 секунд индексации сайта на холодном старте меня устраивают~~ (это я тупанул и не учел, что индексеру еще скомпилироваться надо).
