...
Семантическое. Команда и событие выражают разные сущности. Событие — это бизнес сущность, "что-то произошло", обычно существительное + краткое причастие (например, "заказ создан"). Команда — это приказ, "надо что-то сделать", и чаще это техническая сущность (например, "отправь e-mail").

Количество потребителей и производителей. У команды "потребитель" ровно один — исполнитель команды, а у события — от 0 до бесконечности, причем они могут быть из разных доменов. Команду может дать кто угодно, а произвести событие о том, что "что-то случилось" — только одна сущность.
