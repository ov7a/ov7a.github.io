...
2. Common Closure Principle — SRP для компонента. Новая фича должна затрагивать ровно один компонент. Т.е. классы, которые изменяются по одной причине, должны быть в одном компоненте. "Все классы для работы с БД должны быть в одном компоненте" (sic!).
