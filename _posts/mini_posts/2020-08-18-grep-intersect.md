...
Принцип работы довольно прост: берем паттерны из `file1` (`-f file1`), говорим, что это строки, а не регулярки (`-F`), и ищем точное совпадение строк (`-x`) в `file2`. В интернете пишут, что это быстрее, чем приколы с сортировками и однострочниками на питоне.
