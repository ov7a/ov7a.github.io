...
`@Transactional` [может сломать](https://www.baeldung.com/spring-nosuchbeandefinitionexception#cause-4) любой сервис без интерфейса, причем в лучшем случае будет `NoSuchBeanDefinitionException`, а в худшем, если класс-потребитель не финальный, то будет тупо `null` вместо бина.
