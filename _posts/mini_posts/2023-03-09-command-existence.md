...
tg_id: 384
---
Оказывается, в баше есть встроенная команда `command`, которая изначально предназначена для запуска исполняемого файла, имя которого совпадает с какой-нибудь встроенной командой баша. Однако `command -v` можно использовать для проверки, существует ли исполняемый файл в принципе.
