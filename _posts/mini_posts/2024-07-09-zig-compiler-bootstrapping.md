...
[Подобные выкрутасы](https://en.wikipedia.org/wiki/Bootstrapping_(compilers)) интересны с точки зрения воспроизводимости сборки, точнее, уменьшения количества кода, нужного для этого. В статье в том числе описаны стандартные подходы, которые используются в других языках.
